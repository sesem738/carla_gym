...
import numpy as np
import carla
from gym_carla.agents.tools.misc import get_speed
...
class VehiclePIDController():
    ...
    def run_step(self, target_speed, waypoint):
        ...
        if current_steering >= -0.2 and current_steering <= 0.2:
            steering = 0
        if current_steering > 0.2 and current_steering <= 0.35:
            steering = 0.25
        elif current_steering > 0.35:
            steering = 0.5
        elif current_steering < -0.2 and current_steering >= -0.35:
            steering = -0.25
        elif current_steering < -0.35:
            steering = -0.5
        else:
...
class PIDLateralController():
    ...
    def _pid_control(self, waypoint, vehicle_transform):
        ...
        sskkrr = (np.clip((self._k_p * _dot) + (self._k_d * _de) + (self._k_i * _ie), -1.0, 1.0))
